...
class NeuralToolkitApp:
    ...
    def create_widgets(self):
        ...
        tk.Label(self.root, text="HSType (comma separated)").grid(row=1, column=0, padx=5, pady=1)
        tk.Entry(self.root, textvariable=self.hstype_entry, width=50).grid(row=1, column=1, padx=5, pady=1)

        tk.Label(self.root, text="TS").grid(row=2, column=0, padx=5, pady=1)
        self.ts_scale_widget = tk.Scale(self.root, from_=0, to=25000 * 5 * 60,
                                        resolution=2500,
                                        orient=tk.HORIZONTAL,
                                        length=450,
                                        variable=self.ts_scale)
        self.ts_scale_widget.grid(row=2, column=1, padx=5, pady=1)
        self.ts_scale_widget.bind("<ButtonPress-1>", self.update_scale_limit)

        tk.Label(self.root, text="Samples (25 to 2_500_000)").grid(row=3, column=0, padx=5, pady=1)
        self.samples_scale_widget = tk.Scale(self.root, from_=25, to=25000 * 100,
                                             resolution=25,
                                             orient=tk.HORIZONTAL,
                                             length=450,
                                             variable=self.samples_scale)
        self.samples_scale_widget.grid(row=3, column=1, padx=5, pady=1)

        tk.Label(self.root, text="Sampling Rate").grid(row=4, column=0, padx=5, pady=1)
        tk.Entry(self.root, textvariable=self.sampling_rate_var, width=50).grid(row=4, column=1, padx=5, pady=1)

        tk.Label(self.root, text="Number of Channels").grid(row=5, column=0, padx=5, pady=1)
        tk.Spinbox(self.root, from_=64, to=640, increment=64, textvariable=self.num_channels_var).grid(row=5, column=1, padx=5, pady=1)
        ...
        tk.Label(self.root, text="NTet").grid(row=9, column=0, padx=5, pady=1)
        tk.Spinbox(self.root, from_=0, to=15, textvariable=self.ntet_var).grid(row=9, column=1, padx=5, pady=1)

        tk.Checkbutton(self.root, text="Apply Bandpass Filter", variable=self.filter_var).grid(row=10, columnspan=2, padx=5, pady=1)
