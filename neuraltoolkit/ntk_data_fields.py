...
def get_data_fields(animal=None, field=None, probenum=None, region=None):
        ...
        field : field to return
                field list : 'animal_name', 'species', 'strain', 'genotype', 'sex', 'animal_dob',
        ...
        raises ValueError and RunTimeError
        '''
        ...
        # print(f'field {field}')
        if ((field != 'all') and ((probenum == None) and (region == None)) ):
            raise ValueError(f'Check probenum {probenum} and region {region}')


        response_API = requests.get(f'http://mousecache.wustl.edu/name?name={animal}')
        if response_API.status_code == 200:
            ...
            if ((probenum is not None) and (field != 'all')):
                probe_num = [indx for indx, i  in enumerate(data_animal_json) if i['probe_num'] == probenum][0]
            elif ((region is not None) and (field != 'all')):
                probe_num = [indx for indx, i  in enumerate(data_animal_json) if i['region'] == region][0]
            ...
            if field == 'all':
                ...
                print(f"implant_date {data_animal_json[probe_num]['implant_date']}")
                ...
                print(f"Chan_range {data_animal_json[probe_num]['chan_range']}")
                return data_animal_json[probe_num]['chan_range']
            elif field == 'location':
                print(f"Location AP {data_animal_json[probe_num]['ap']} ML {data_animal_json[probe_num]['ml']} DV {data_animal_json[probe_num]['dv']}")
